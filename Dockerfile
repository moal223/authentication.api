...
COPY . .
WORKDIR "/src/AuthenticationServer.Api"
RUN dotnet build "AuthenticationServer.Api.csproj" -c $BUILD_CONFIGURATION -o /app/build

FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "AuthenticationServer.Api.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
